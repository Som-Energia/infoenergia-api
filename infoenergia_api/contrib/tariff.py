...
class TariffPrice(object):
    ...
    geographical_region = {"canarias": 1646, "peninsula": 3830}
    ...
    async def create(cls, filters, tarif_id=None):
        ...
        self = await loop.run_in_executor(None, cls, filters, tarif_id)
        return self
        ...
        return (
            {
                ...
                "autoconsumo": dict(
                    sorted(self.price_data["energia_autoconsumida"].items())
                ),
                ...
                if self.price_data["fiscal_position"]
                else {},
            }
            if self.price_data
            else {}
    ...
    def tariff(self):
        ...
        if "error" in tariff_prices.keys():
            return {
                "error": tariff_prices.get("error", False),
        ...
        if tariff_prices:
            if self.contract_id:
                ...
                return {
                    "tariffPriceId": self.tariff_id,
                    "prices": {"current": current, "history": history},
                }
