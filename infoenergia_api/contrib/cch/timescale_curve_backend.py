...
class TimescaleCurveBackend:
        ...
        **extra_filter,
        ...
        async with AsyncClientCursor(erpdb) as cursor:
            ...
            if start:
                ...
                result += [cursor.mogrify("utc_timestamp >= %s", [start_utc])]
            if end:
                end_utc = local_isodate_2_utc_isodatetime(increment_isodate(end))
                result += [cursor.mogrify("utc_timestamp <= %s", [end_utc])]
            if downloaded_from:
                result += [
                    cursor.mogrify("create_at >= %s", [downloaded_from + " 00:00:00"])
                ]
            if downloaded_to:
                result += [
                    cursor.mogrify("create_at <= %s", [downloaded_to + " 00:00:00"])
                ]
    ...
    async def get_curve(
        ...
        async with AsyncClientCursor(erpdb, row_factory=dict_row) as cursor:
            ...
            return [cch_transform(cch) for cch in await cursor.fetchall()]
